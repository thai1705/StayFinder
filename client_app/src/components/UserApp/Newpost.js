...
function NewPost() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/listpost/3`)
      .then((res) => res.json())
      .then((data) => {
        setPost(data);
  ...
  return (
    <div >
      ...
      <div style={{ marginBottom: '20px' }}>
        <p style={{ marginBottom: '15px', fontWeight:'bold', fontSize:'20px' }}>TIN MỚI</p>
            ...
            return (
             ...
             <p> {countimages} </p>
                ...
                <div style={{ padding: '0px 10px' }}>
                  <p style={{ color: '#2E73A0', fontWeight: 'bold', overflowWrap: 'break-word', marginTop: '10px', textTransform:'uppercase', fontSize:'15px' }}>{post.title}</p>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <p style={{color:'#2E73A0'}}>{post.room_price} triệu/tháng</p>
