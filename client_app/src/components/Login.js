  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const userId = data.userId; // Giả sử userId được trả về từ API đăng nhập
        await fetch(`http://localhost:8000/api/auth/update-status/${userId}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${data.token}`, // Thêm token nếu cần
          },
          body: JSON.stringify({ status: 1 }), // Cập nhật trạng thái thành 1
        });
